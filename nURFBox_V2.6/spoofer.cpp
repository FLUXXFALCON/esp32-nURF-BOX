...
void updateDisplay() {
  ...
  switch (deviceType) {
    ...
    case 5:
      ...
      break;
    case 6:
      u8g2.drawStr(x, y, "[ Airpods Pro Gen 2 ]");
      break;
    case 7:
...
void toggleAdvertising() {
  if (pAdvertising == nullptr) {
    u8g2.clearBuffer();
    u8g2.setFont(u8g2_font_profont11_tf);
    u8g2.drawStr(0, 30, "BLE Advertising not initialized!");
    u8g2.sendBuffer();
  ...
  if (!isAdvertising) {
    ...
    if (attack_state == 1) {
      ...
      for (int i = 0; i < 6; i++) {
        dummy_addr[i] = esp_random() % 256; // Daha güvenli rastgele sayı
        if (i == 0) {
          dummy_addr[i] |= 0xF0;
        }
      }
      ...
      // Set advertisement type based on advType
      switch (advType) {
        case 1:
...
void spooferSetup() {
  ...
  if (pAdvertising == nullptr) {
    u8g2.clearBuffer();
    u8g2.setFont(u8g2_font_profont11_tf);
    u8g2.drawStr(0, 30, "Advertising is null!");
    u8g2.sendBuffer();
    Serial.println("Advertising is null!");
