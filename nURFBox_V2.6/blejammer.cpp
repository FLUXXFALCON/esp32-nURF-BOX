...
RF24 radio1(CE_PIN_1, CSN_PIN_1, 12500000);
RF24 radio2(CE_PIN_2, CSN_PIN_2, 12500000);
RF24 radio3(CE_PIN_3, CSN_PIN_3, 12500000);
...
unsigned long lastPowerDownTime = 0;
const unsigned long powerDownInterval = 100; // delay(100) yerine kullanılacak
...
void initializeRadios() {
  if (currentMode != DEACTIVE_MODE) {
    ...
    unsigned long currentTime = millis();
    if (currentTime - lastPowerDownTime >= powerDownInterval) {
      radio1.powerDown();
      radio2.powerDown();
      radio3.powerDown();
      lastPowerDownTime = currentTime;
    }
  } 
}
